...
import { Mail, PlusCircle, Trash2, RefreshCw, CheckCircle } from 'lucide-react';
...
const initialAccounts: LinkedAccount[] = [
  ...
  { id: 'gmail2', provider: 'gmail', email: 'personal.subscriptions@gmail.com', lastScan: new Date(Date.now() - 86400000 * 5).toISOString() },
];
...
export default function ConnectAccountPage() {
  ...
  const handleConnectGmail = () => {
    ...
    setTimeout(() => {
      const newAccountEmail = `new.account${linkedAccounts.length + 1}@gmail.com`;
  ...
  const handleScanAccount = (accountId: string) => {
    ...
    toast({ title: `Scanning ${linkedAccounts.find(a=>a.id===accountId)?.email}...` });
    // Simulate scanning process
    setTimeout(() => {
      setLinkedAccounts(prev => prev.map(acc => acc.id === accountId ? { ...acc, isScanning: false, lastScan: new Date().toISOString() } : acc));
      toast({ title: 'Scan Complete!', description: `Finished scanning ${linkedAccounts.find(a=>a.id===accountId)?.email}.` });
