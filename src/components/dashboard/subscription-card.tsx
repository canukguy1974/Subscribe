...
export default function SubscriptionCard({ subscription, onEdit, onDelete, onToggleNotification }: SubscriptionCardProps) {
  ...
  const nextBillingDate = subscription.nextBillingDate ? parseISO(subscription.nextBillingDate) : null;
  const trialEndDate = subscription.trialEndDate ? parseISO(subscription.trialEndDate) : null;
  const today = new Date();
  
  let daysUntilNextPayment: number | null = null;
  if (nextBillingDate && isValid(nextBillingDate)) {
    daysUntilNextPayment = differenceInDays(nextBillingDate, today);
  }

  let daysUntilTrialEnd: number | null = null;
  if (trialEndDate && isValid(trialEndDate)) {
    daysUntilTrialEnd = differenceInDays(trialEndDate, today);
  }
  
  const paymentOrTrialStatus = () => {
    if (trialEndDate && isValid(trialEndDate)) {
      if (daysUntilTrialEnd !== null && daysUntilTrialEnd <= 2 && daysUntilTrialEnd >= 0) {
      ...
      } else if (daysUntilTrialEnd !== null && daysUntilTrialEnd < 0) {
    ...
    if (nextBillingDate && isValid(nextBillingDate)) {
      if (daysUntilNextPayment !== null && daysUntilNextPayment <= 2 && daysUntilNextPayment >= 0) {
        ...
        <div className="flex justify-between items-start">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => onDelete(subscription.id)} className="text-destructive focus:text-destructive focus:bg-destructive/10">
      ...
      <CardFooter className="flex justify-between items-center border-t pt-4 mt-auto">
        <div className="flex items-center space-x-2">
          <Switch
            ...
            onCheckedChange={(checked) => onToggleNotification(subscription.id, checked)}
