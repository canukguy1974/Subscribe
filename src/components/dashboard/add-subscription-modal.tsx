...
export default function AddSubscriptionModal({ isOpen, onOpenChange, onSave, existingSubscription }: AddSubscriptionModalProps) {
  ...
  const onSubmit = (values: AddSubscriptionFormValues) => {
    const subscriptionData: Subscription = {
      ...
      userId: existingSubscription?.userId || 'user123', // Replace with actual user ID
      ...values,
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 px-1">
            ...
            <DialogFooter>
              ...
              <Button type="submit" className="bg-gradient-to-r from-primary to-accent text-primary-foreground hover:opacity-90 transition-opacity" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting ? (existingSubscription ? "Saving..." : "Adding...") : (existingSubscription ? 'Save Changes' : 'Add Subscription')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
